...
def get_sqlcursor_mgcollection():
    ...
    return sqldb,sqlcursor,mgcollection


def get_ymtrend(data):
    # 连接数据库
    sqldb,sqlcursor,mgcollection=get_sqlcursor_mgcollection()
    ...
    for tag in data.keys():
        ...
        try:
            sql = "SELECT YEAR(CreationDate) tagYear,\
                        ...
                        FROM posts \
                        WHERE YEAR(CreationDate)>2014 AND Tags like '%<{0}>%' \
...
def get_data_slice(data,interval=10000):
    ...
    if leng == 50000:
        alltags = alltags[30000:50000]
        leng = 20000
    for i in range(0,leng//interval):
        dic={}
        tagnames=alltags[i*interval:(i+1)*interval]
...
def main(minidata):
    dataslice=get_data_slice(minidata,interval=200)
    dpool = dPool(processes=10)
    dpool.map(get_ymtrend, [dataslice[i] for i in range(0, 10)])
...
if __name__ == '__main__':
    ...
    mpool.map(main,[datalist[i] for i in range(0,10)])
    # get_ymtrend(bigdata)

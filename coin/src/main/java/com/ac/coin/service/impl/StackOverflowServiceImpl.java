...
public class StackOverflowServiceImpl implements StackOverflowService {
    ...
    public List<NodeVO> findDefaultStackNode(){
        List<Tags> tags = stackOverflowDAO.findHottestTags(0);
        List<NodeVO> nodeVOS = new ArrayList<>();
        int i=1;
        for(Tags t:tags){
            if(StackTrans.has(nodeVOS,t.getId())) continue;
            nodeVOS.add(StackTrans.nodeVO(t,i));
            i=i+1;
        }
        return nodeVOS;
    }
    ...
    public List<RelationVO> findDefaultStackRelation(){
        List<RelationshipValue> relationships = stackOverflowDAO.findHottestRelations(0);
        List<RelationVO> relationVOS = new ArrayList<>();
        for(RelationshipValue r:relationships){
            relationVOS.add(Transform.relationVO(Transform.relationshipValueToRelation(r)));
        }
        return relationVOS;
    }
